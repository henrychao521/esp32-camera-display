...
所以**接線與 `camera_pins.h` 都不用改**。唯一必要的程式修改:

- **XCLK 時脈**:把頂部 `#define CAM_XCLK_HZ` 設為 **20000000**(OV5640 建議 20MHz;
  OV2640 用 10000000)。若換 OV5640 後看到損毀影格,再試著調低。
- **畫面方向**:OV5640 安裝方向若與 OV2640 不同,調 `applySensor()` 的 `set_vflip`/`set_hmirror`。

選配(可下一步再做):
- **自動對焦**:OV5640 有 VCM 對焦馬達,需另裝 `ESP32_OV5640_AF` 函式庫並上傳對焦韌體。
- **高解析度拍照**:OV5640 最高 QSXGA(2592×1944);可只在拍照時切到高解析度,
